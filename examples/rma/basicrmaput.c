...
int main(int argc, char **argv)
{
  ...
  if (myid == 0){
    /* MPI_BOTTOM: this rank makes no memory available to recv data */
    MPI_Win_create(MPI_BOTTOM, 0, sizeof(int), MPI_INFO_NULL, comm, &win);

  } else if (myid == 1){
    MPI_Win_create(recvbuf, count*sizeof(double), sizeof(double), MPI_INFO_NULL, comm, &win);
  }
  ...
  if( myid == 0 ){
    MPI_Put(sendbuf, count, MPI_DOUBLE, 1, 0, count, MPI_DOUBLE, win);
  }
