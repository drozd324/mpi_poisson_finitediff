...
int main(int argc, char* argv[])
{
  ...
  if( argc != 3 ){
    fprintf(stderr, "Usage: %s <number of processors> <number of integers per processor>\n", argv[0]);
    exit(1);
  }
  nprocs = atoi(argv[1]);
  ...
  sprintf(fname, "intmpiotfile_%d_%d",nprocs,items_per_proc);
  ...
  data = (int *)malloc(totalitems * sizeof(int));
  ...
  ret = fwrite(data, sizeof(int), totalitems, fp);
